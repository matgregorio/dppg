...
<?php
include ('../simposio2013/../simposio2013/includes/config.php');
...
if ($sa == 't') {
  $sql_trab = "SELECT a.codigo_trab, a.codigo_ano, a.codigo_acervo, t.* FROM acervo AS a, trabalhos AS t WHERE a.codigo_ano='$ano' AND (t.codigo_trab LIKE '%$titulo%' AND t.codigo_trab LIKE '%$palavra_chave%' AND t.autor1 LIKE '%$autor1%' AND t.acervo='1') AND a.codigo_trab=t.codigo_trab ORDER BY t.titulo";
} else {
  $sql_trab = "SELECT a.codigo_trab, a.codigo_ano, a.codigo_acervo, t.* FROM acervo AS a, trabalhos AS t WHERE a.codigo_ano='$ano' AND (t.codigo_trab LIKE '%$titulo%' AND t.codigo_trab LIKE '%$palavra_chave%' AND t.autor1 LIKE '%$autor1%' AND t.acervo='1') AND a.codigo_trab=t.codigo_trab AND t.codigo_sa='$sa' ORDER BY t.titulo";
}
        ...
        if (mysql_num_rows($query_trab) > 0) {
          ...
          while ($campos = mysql_fetch_array($query_trab)) {
            ...
            <div class="tgl">
              ...
              <?php echo"<a href='http://sistemas.riopomba.ifsudestemg.edu.br/dppg/simposio2013/gerar_pdf_trabalhos.php?codigot=$campos[codigo_trab]' target='_blanck' align='rigth'><img src='images/pdf.png'> Gerar PDF</a>"; ?>
            </div>
            <hr>
            <?php
          }
