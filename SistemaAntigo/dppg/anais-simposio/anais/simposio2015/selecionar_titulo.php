...
include('/var/www/html/simposio2015/includes/config.php');
...
if ($sa != "t") {
  ...
  $query_ap = mysql_query("SELECT DISTINCT p.nome, p.cpf FROM participantes p, trabalhos t, acervo ac WHERE p.cpf=t.autor1 AND t.codigo_trab=ac.codigo_trab AND t.codigo_sa=$sa AND ac.codigo_ano='8' ORDER BY p.nome");
} else {
  ...
  $query_ap = mysql_query("SELECT DISTINCT p.nome, p.cpf FROM participantes p, trabalhos t, acervo ac WHERE p.cpf=t.autor1 AND t.codigo_trab=ac.codigo_trab AND ac.codigo_ano='8' ORDER BY p.nome");
}
...
//  echo "<td>";
  ...
  echo "</table>";
  echo "<br>";
  echo "<input type='hidden' name='data' value='anais/simposio2015/acervo_new'>";
