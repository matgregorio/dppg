...
if ($_SESSION[anais_logado] == TRUE) {
  if ($_POST[ok] == 'ok') {
    $altData = new AltData();
    $oInicio = $altData->AlterarInicio($_POST[code], $_POST[inicio]);
    unset($altData);
  } else {
  ...
  <section id="content" class="primary" role="main">
    <div id="post-5" class="post-5 page type-page status-publish hentry">
      <h2 class="page-title">Editar Conteúdo do Início</h2>
      <div class="entry clearfix">
        <p style="text-align: justify;">
          ...
          if ($_POST[ok] == "ok") {
            ...
            ?>
          <form name="altInicio" action="index.php?data=view/altInicio" method="POST">
            <center><input type="submit" name="salar" value="Salvar"/></center><br>
            <input type="hidden" name="ok" value="ok"/>
            <input type="hidden" name="code" value="<?php echo $oInicio->getCodigo() ?>"/>
            <textarea name="inicio" style="width: 100%; min-height: 40%">
              <?php echo $oInicio->getTexto(); ?>
            </textarea>
          </form>
        <?php } ?>
        </p>
