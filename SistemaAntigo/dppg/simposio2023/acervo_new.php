<html>
  ...
  <body>
    <div id="box-toggle">
      ...
      if ($titulo != '') {
        if ($sa == 't') {
          $sql_trab = "SELECT a.codigo_trab, a.codigo_ano, a.codigo_acervo, t.* FROM acervo AS a, trabalhos AS t WHERE a.codigo_ano='$ano' AND (t.codigo_trab = '$titulo' AND t.palavra_chave LIKE '%$palavra_chave%' AND t.acervo='1') AND a.codigo_trab=t.codigo_trab ORDER BY t.titulo";
        } else {
          $sql_trab = "SELECT a.codigo_trab, a.codigo_ano, a.codigo_acervo, t.* FROM acervo AS a, trabalhos AS t WHERE a.codigo_ano='$ano' AND (t.codigo_trab = '$titulo' AND t.palavra_chave LIKE '%$palavra_chave%' AND t.acervo='1') AND a.codigo_trab=t.codigo_trab AND t.codigo_sa='$sa' ORDER BY t.titulo";
        }
      }else{
        if ($sa == 't') {
          $sql_trab = "SELECT a.codigo_trab, a.codigo_ano, a.codigo_acervo, t.* FROM acervo AS a, trabalhos AS t WHERE a.codigo_ano='$ano' AND (t.palavra_chave LIKE '%$palavra_chave%' AND t.acervo='1') AND a.codigo_trab=t.codigo_trab ORDER BY t.titulo";
        } else {
          $sql_trab = "SELECT a.codigo_trab, a.codigo_ano, a.codigo_acervo, t.* FROM acervo AS a, trabalhos AS t WHERE a.codigo_ano='$ano' AND (t.palavra_chave LIKE '%$palavra_chave%' AND t.acervo='1') AND a.codigo_trab=t.codigo_trab AND t.codigo_sa='$sa' ORDER BY t.titulo";
        }
      }
//      echo $sql_trab;
      ...
      if (mysql_num_rows($query_trab) > 0) {
        echo "<center><br><b>Resultado do Sumário de $campos_ano[ano]</b><br><br></center>";
        echo '<center><a href="simposio.php">Voltar</a></center>';
        echo '<br><br>';
        echo "<font><b><u>Código - Título:</u></b></font>";
        $cor = "#95e197";
        while ($campos = mysql_fetch_array($query_trab)) {
          ...
          echo "<span><p><img src='images/plus.gif' />&nbsp;<b>$campos[codigo_trab]</b> - <b>$titulo</b></p></span>";
...
//          $resumo = str_replace("</p>", "", $resumo);
          ...
          <div class="tgl">
            ...
            <?php echo "<a href='../simposio2015/../simposio2015/gerar_pdf_trabalhos.php?codigot=$campos[codigo_trab]' target='_blanck' align='rigth'><img src='images/pdf.png'> Gerar PDF</a>"; ?>
          </div>
          <hr>
          <?php
        }
