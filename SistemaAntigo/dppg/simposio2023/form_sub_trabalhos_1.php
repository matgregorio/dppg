<?php
if ($_SESSION[logado_simposio_2021] && $_SESSION[cpf] == '08723823623') {
  ?>
  <head>
...
// Calcula a diferença de dias
    ...
    $dias = $resultado;
    if ($modalidade == "N") {
      $sql = "INSERT INTO trabalhos (data_envio, dias_restantes, situacao, autor1, autor2, autor3, autor4, autor5, autor6, autor7, cpf_prof_analisador, titulo, resumo, palavra_chave, modalidade, codigo_sa, cpf, aprovado, apresentador) VALUES ('$data_atual', '$dias', '$situacao', '$cpf_autor1', '$cpf_autor2', '$cpf_autor3', '$cpf_autor4', '$cpf_autor5', '$cpf_autor6','$cpf_autor7', '$cpfanalisador', '$titulo', '$resumo', '$palavra_chave', '$modalidade', '$subarea' , '$cpf_autor1', '$aprovado', '$apresentador')";
    } else {
...
//    echo $sql;
    ...
    if ($res == 1) {
      ...
      $message = 'Prezado Orientador,
                    houve uma submissão de trabalho que deve ser avaliada pelo senhor (a).
					Para acessar segue o link abaixo.
					http://sistemas.riopomba.ifsudestemg.edu.br/dppg/simposio2021
					Acesso Simpósio
					Login: ' . $campos1[cpf] . ' 
					A senha é a mesma que você cadastrou no sistema. 
					Caso não lembre acesse o link abaixo para mudar a senha
					http://sistemas.riopomba.ifsudestemg.edu.br/dppg/simposio2021/simposio.php?arquivo2=form_envia_senha.php
					Setor DPPG';
      ...
      $headers = 'From: dppg.riopomba@ifsudestemg.edu.br' . "\r\n";
      $headers .= "Cc: suport.simposio@gmail.com";
...
//$sql1 = "select t.codigo_trab, t.situacao, t.cpf_prof_analisador, t.autor1 , t.titulo, t.aprovado, t.presenca, s.nome_sa from trabalhos t, sub_area s where s.codigo_sa = t.codigo_sa and t.cpf = $_GET[a]";
  $a = mysql_real_escape_string($_GET[a]);
  $sql1 = "select t.*, s.nome_sa from trabalhos t, sub_area s where s.codigo_sa = t.codigo_sa and ($a = autor1 or $a = autor2 or $a = autor3 or $a = autor4 or $a = autor5 or $a = autor6 or $a = autor7 or cpf_prof_analisador=$a)";
  $resultado1 = mysql_query($sql1);
  if (mysql_num_rows($resultado1) != 0) {
    ...
    <td>
    <center><b><font color="#FFFFFF">&nbsp;Nota Apresentação&nbsp;</font></b></center>
    </td>
    <td>
    ...
    while ($campos1 = mysql_fetch_array($resultado1)) {
      ...
      if ($campos1[aprovado] == 0) {
        echo '<font color="#FF0000"><center>Trabalho não Aprovado pelo Avaliador</center></font>';
      ...
      if ((mysql_num_rows($result) > 0) && ($campos1[aprovado] == 2) && ($campos1[autor1] == $_GET[a])) {
      ...
      if ($campos1[nota1] != '' && $campos1[nota2] != '') {
        echo "<td align='center'>" . (($campos1[nota1] + $campos1[nota2]) / 2) . " pts</td>";
      } elseif ($campos1[nota1] != '' && $campos1[nota2] == '') {
        echo "<td align='center'>$campos1[nota1] pts</td>";
      } elseif ($campos1[nota1] == '' && $campos1[nota2] != '') {
        echo "<td align='center'>$campos1[nota2] pts</td>";
      } else {
      ...
      echo "<table border='0' id='avaliadores$campos1[codigo_trab]' width='90%' style='display: none'>";
      $cont = 0;
      $total = 0;
      $controle = 0;
      if (mysql_num_rows($resultado_avaliador) > 0) {
        ...
        if (($total / $cont) < 60) {
          echo "<td><b>Média Final:</b><font color='#ff0000'>" . ($total / $cont) . "</font></td>";
        } else {
          echo "<td><b>Média Final:</b>" . ($total / $cont) . "</td>";
        }
...
//  if (mysql_num_rows(mysql_query("SELECT cpf FROM trabalhos WHERE cpf=$_SESSION[cpf]")) < 2) {//ñ é necessário para burlar
  if (1 == 1) {
    ...
    $cpf = mysql_real_escape_string($_GET[a]);
...
//    } else if ((date("Ymd") >= $data_inicio) && (date("Ymd") <= $data_fim)) {
    if (1 == 1) {
      ...
      <br>
      <center>
        <form name="form_submissao" method="post" onsubmit="javascript: return checkcontatos()"
              action="simposio.php" enctype="multipart/form-data">
          <table border="0" width="100%" class="esquerda">
            <tr>
              <td><?php echo '<input type="hidden" name="cpf1" size="11" maxlength="11" value="' . $_GET[a] . '">'; ?></td>
            </tr>
            <tr>
            ...
            <tr>
              <td>Este resumo refere-se ao seu<br>projeto de Iniciação Científica ou<br>Pós-Graduação Institucional?</td>
              <td>
                <input type="radio" onclick="script : mostrarIC(this.value)"
                       onchange="script : mostrarIC(this.value)" name="modalidade" value="S">Sim
                <input type="radio" onclick="script : mostrarIC(this.value)"
                       onchange="script : mostrarIC(this.value)" name="modalidade" value="N"
                       checked="true">Não
              </td>
            </tr>
            <tr><br></tr>
            <tr id="tipoIC" style="display: none">
              ...
              <td>
                <input type="radio" name="tipoIniciacao" value="T" checked="true">Técnico
                <input type="radio" name="tipoIniciacao" value="G">Graduação
                <input type="radio" name="tipoIniciacao" value="M">Mestrado
              </td>
            </tr>
            <tr><br></tr>
            <tr>
