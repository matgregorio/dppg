...
if (in_array("1", $_SESSION[codigo_grupo]) || in_array("2", $_SESSION[codigo_grupo])) {
  ...
  <body>
    ...
    /*Faz a segurança para que ninguém acesse outro trabalho senão os que esteja avaliando*/

    $cpfProfessor = $_SESSION[cpf];
    $querySeguranca = "SELECT codigo_trab FROM `trabalhos` where cpf_prof_analisador = $cpfProfessor";
    $resultadoQSeguranca = mysql_query($querySeguranca);

    while($trabalhosAcessiveis = mysql_fetch_array($resultadoQSeguranca))
    {
        if (in_array($codigopost, $trabalhosAcessiveis))
        {
            $liberado = true;
            break;
        }
    ...
    if ($_POST[submissao] == 'S') {
      ...
      <div id="conteudo3" width="300">
       ...
       //$autor7 = mysql_real_escape_string($_POST[autor7]);
        ...
        echo "<meta http-equiv='refresh' content='2;URL=resumo.php?codigo=$codigopost'/>";
        ?>
      </div>
      <?php
    } else {
