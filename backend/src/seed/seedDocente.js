...
async function seedDocente() {
  try {
    ...
    const docenteExistente = await Docente.findOne({ cpf: '123.456.789-00' });
    
    // Verificar se o usuário já existe (usando .select('+senha') para poder atualizar)
    let user = await User.findOne({ email: 'docente@gov.br' }).select('+senha');
    
    if (docenteExistente && user) {
      console.log('⚠️  Docente já existe no banco de dados');
    ...
    if (user) {
      console.log('⚠️  Usuário docente@gov.br já existe');
      process.exit(1);
    }
    ...
    user = await User.create({
      nome: 'Prof. Dr. João da Silva',
      email: 'docente@gov.br',
      cpf: '123.456.789-00',
